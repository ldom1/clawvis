...
from style_guide import (
    ...
    update_style_guide_from_reverse_prompt,
)
...
Status: All systems green. Next check at 16:00."""
    ...
    style = update_style_guide_from_reverse_prompt(
        example_text=perfect_report, name="operational-reports"
    )
...
def example_5_use_global_router():
    ...
    _task1 = router.route_task(
        task_id="task1", instruction="Generate report", agent_id="reporter"
    )

    _task2 = router.route_task(
        task_id="task2", instruction="Analyze metrics", agent_id="analyzer"
    )
