...
class MammouthAIClient:
    ...
    def __init__(self, api_key: Optional[str] = None, model: str = None):
        ...
        if not self.api_key:
            raise ValueError("MAMMOUTH_API_KEY not configured in .env or passed as argument")
    ...
    def complete(
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"MammouthAI error {response.status_code}: {response.text}")
                return {
                    "success": False,
                    "error": response.text,
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            if result.get("success"):
                ...
                logger.error(f"❌ MammouthAI connection test FAILED: {result.get('error')}")
                return False
