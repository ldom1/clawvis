...
def test_saved_projects_root_overrides_env(tmp_path, monkeypatch):
    settings_path = tmp_path / "kanban" / "hub_settings.json"
    settings_path.parent.mkdir(parents=True)
    settings_path.write_text(
        json.dumps({"linked_instances": [], "projects_root": "/home/lgiron/lab/projects"}),
        encoding="utf-8",
    )
    monkeypatch.setattr(core, "HUB_SETTINGS_FILE", settings_path)
    monkeypatch.setenv("PROJECTS_ROOT", "/home/lgiron/lab_perso/projects")

    out = core.get_hub_settings()

    assert out["projects_root"] == "/home/lgiron/lab/projects"
