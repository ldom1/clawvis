...
trap 'e=$?; [ $e -ne 0 ] && [ -n "${LOGGER_CORE:-}" ] && [ -d "$LOGGER_CORE" ] && clawvis_uv_run_dir "$LOGGER_CORE" dombot-log "ERROR" "cron:proactive-innovation" "system" "cron:fail" "Script failed (exit $e)" 2>/dev/null || true; exit $e' EXIT
...
report=$(clawvis_uv_run_dir "$CORE_DIR" python -m proactive_innovation 2>>"$LOG") || { echo "[$(date)] Run failed" >>"$LOG"; exit 1; }
if [ -n "$report" ]; then
