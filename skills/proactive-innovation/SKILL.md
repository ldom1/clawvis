...
~/.openclaw/skills/proactive-innovation/scripts/run-proactive-innovation.sh

# Option 2 — Module Python direct
uv run --directory ~/.openclaw/skills/proactive-innovation/core python -m proactive_innovation
```
...
- `~/.openclaw/workspace/memory/projects/*.md` (projets actifs, hors `_template`)
- Champ `path` dans le frontmatter (repo à scanner)
- `memory/resources/knowledge/`, `memory/resources/curiosity/` (pour idées)
- `memory/caps/entrepreneur.md` (destination idées)
- **Kanban API** :
...
- Lire `memory/resources/knowledge/` et `memory/resources/curiosity/` (ou synthèses récentes).
- Proposer des idées courtes (entreprise, OSS, side-project) et les écrire dans `memory/caps/entrepreneur.md` (section **Idées (propositions auto)**) ou dans `memory/resources/ideas/YYYY-MM-DD.md`.
...
| **knowledge_consolidator** | MEMORY.md, QMD, daily/curiosity, consolidation |
