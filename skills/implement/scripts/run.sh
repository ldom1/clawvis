...
trap 'e=$?; [ $e -ne 0 ] && [ -n "${LOGGER_CORE:-}" ] && [ -d "$LOGGER_CORE" ] && uv run --directory "$LOGGER_CORE" dombot-log "ERROR" "skill:implement" "system" "impl:fail" "Script failed (exit $e)" 2>/dev/null || true; exit $e' EXIT
