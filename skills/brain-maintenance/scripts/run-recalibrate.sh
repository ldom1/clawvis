...
trap 'e=$?; [ $e -ne 0 ] && [ -n "${LOGGER_CORE:-}" ] && [ -d "$LOGGER_CORE" ] && clawvis_uv_run_dir "$LOGGER_CORE" dombot-log "ERROR" "cron:brain-maintenance" "system" "recalibrate:fail" "Brain maintenance recalibrate failed (exit $e)" 2>/dev/null || true; exit $e' EXIT
