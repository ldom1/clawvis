...
trap 'e=$?; [ $e -ne 0 ] && [ -n "${LOGGER_CORE:-}" ] && [ -d "$LOGGER_CORE" ] && uv run --directory "$LOGGER_CORE" dombot-log "ERROR" "cron:brain-maintenance" "system" "trim:fail" "Brain maintenance trim failed (exit $e)" 2>/dev/null || true; exit $e' EXIT
...
if ! uv run --directory "$CORE_BRAIN" python -m brain_maintenance trim; then
  SUMMARY=$(uv run --directory "$CORE_BRAIN" \
    python -c "
