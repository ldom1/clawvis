...
| 21 | `doctor` : *Skipping skill path that resolves outside its configured root* (répété) | Symlinks `~/.openclaw/skills/<name>` → `Lab/clawvis/...` : la racine gérée est `~/.openclaw/skills`, OpenClaw ignore les cibles hors racine | **`clawvis skills sync`** (`scripts/sync-openclaw-skills-dirs.sh`) : écrit `skills.load.extraDirs`, supprime les symlinks managés, redémarre **`openclaw-gateway`** (user systemd) si présent. Les **crons** doivent utiliser des chemins absolus vers `…/clawvis/skills/…` (pas `~/.openclaw/skills/…`) — mise à jour manuelle de `jobs.json` si besoin |
